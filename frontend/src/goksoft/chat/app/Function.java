...
public class Function {
        ...
        Thread thread = new Thread(()-> {
            ...
            while(selv == currentTimer && GlobalVariables.isThread) {
                try {
                    ...
                    Get all messages.*/
                    String cevap = ServerFunctions.HTMLRequest(ServerFunctions.serverURL + "/messages/check-notif", "receiver=" + ServerFunctions.encodeURL(GlobalVariables.getLoggedUser()) + "&chatter=" + curFriend);
    ...
    public static void getProfilePhoto(boolean mouse) {
        ...
        Platform.runLater(() -> {
            if (image == null || image.isError()){ //Fill profile photo in blue if image has error
    ...
    public static void getFriends(){
        ...
        Thread thread = new Thread(() -> {
            try {
                String stringArray = ServerFunctions.HTMLRequest(ServerFunctions.serverURL + "/friends/get", "username=" + ServerFunctions.encodeURL(GlobalVariables.getLoggedUser()));
    ...
    public static void getFriendRequests(){
        ...
        Thread thread = new Thread(() -> {
            try {
                String stringArray = ServerFunctions.HTMLRequest(ServerFunctions.serverURL + "/friends/requests", "username=" + ServerFunctions.encodeURL(GlobalVariables.getLoggedUser()));
                System.out.println(stringArray);
                JSONParser jsonParser = new JSONParser();
                JSONArray jsonArray = (JSONArray) jsonParser.parse(stringArray);
    ...
    public static void sendMessage(){
        ...
        Thread thread = new Thread(() ->{
            ...
            String response = ServerFunctions.HTMLRequest(ServerFunctions.serverURL + "/messages/send","sender=" + ServerFunctions.encodeURL(GlobalVariables.getLoggedUser()) + "&receiver=" + curFriend + "&message=" + msg);
    ...
    public static void getMessages(){
        ...
        Thread thread = new Thread(() -> {
            ...
            try {
                ...
                stringArray = ServerFunctions.HTMLRequest(ServerFunctions.serverURL + "/messages/get", "user1=" + ServerFunctions.encodeURL(GlobalVariables.getLoggedUser()) + "&receiver=" + curFriend);
                JSONParser jsonParser = new JSONParser();
                JSONArray jsonArray = (JSONArray) jsonParser.parse(stringArray);
                for (int i = 0; i < jsonArray.size(); i++){
    ...
    public static void changeProfilePhoto(MouseEvent event){
            ...
            if (file != null){
                try {
                    ...
                    String string = ServerFunctions.FILERequest(ServerFunctions.serverURL + "/users/photo?username=", file, "photo");
    ...
    public static void searchOnUsers(KeyEvent event){
        ...
        Thread thread = new Thread(() -> {
            try {
                ...
                String stringArray = ServerFunctions.HTMLRequest(ServerFunctions.serverURL + "/users/search","username=" + writtenName);
                System.out.println(stringArray);
                JSONParser jsonParser = new JSONParser();
                JSONArray jsonArray = (JSONArray) jsonParser.parse(stringArray);
