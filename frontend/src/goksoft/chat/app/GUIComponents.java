...
public class GUIComponents {
    ...
    public static Image returnPhoto(String username){
        ...
        Image imagefx = null;
        try {
            BufferedImage image = ImageIO.read(new URL(ServerFunctions.serverURL + "/users/photo/" + imageName));
            if (image != null) {
    ...
    private static EventHandler addFriendEvent(String userName){
        EventHandler event = (EventHandler<?>) mouseEvent -> {
            ...
            String response = ServerFunctions.HTMLRequest(ServerFunctions.serverURL + "/friends/send-request", "sender=" + ServerFunctions.encodeURL(GlobalVariables.getLoggedUser()) + "&receiver=" + receiverUser);
    ...
    private static EventHandler acceptFriendEvent(String senderName) {
        ...
        EventHandler event = (EventHandler<?>) mouseEvent -> {
            String response = ServerFunctions.HTMLRequest(ServerFunctions.serverURL + "/friends/accept", "adder=" + ServerFunctions.encodeURL(GlobalVariables.getLoggedUser()) + "&added=" + addedName);
            System.out.println(response);
            if (response.equals("addfriend successful")) {
                WarningWindowController.warningMessage("Friend added!");
            } else if (response.equals("addfriend unsuccessful")) {
    ...
    private static EventHandler rejectFriendEvent(String senderName) {
        ...
        EventHandler event = (EventHandler<?>) mouseEvent -> {
            String response = ServerFunctions.HTMLRequest(ServerFunctions.serverURL + "/friends/reject", "blocker=" + ServerFunctions.encodeURL(GlobalVariables.getLoggedUser()) + "&blockedUser=" + blockedName);
            System.out.println(response);
            if (response.equals("rejection successful")){
                WarningWindowController.warningMessage("Request rejected!");
            } else if (response.equals("rejection unsuccessful")){
